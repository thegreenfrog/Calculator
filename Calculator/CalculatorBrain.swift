...
class CalculatorBrain{
    ...
    private func buildDesc(ops: [Op]) -> (result: String, restOp: [Op])
    {
        if !ops.isEmpty
        {
            var remainingOp = ops
    ...
    private func findErrors(ops: [Op]) -> (result: Bool, type: String, value: Double?, restOp: [Op])
    {
        if !ops.isEmpty
        {
            ...
            case .BinaryOperation(let sign, let bOp):
